...
```
yarn add baileys-api
```

Use the edge version (no guarantee of stability, but latest fixes + features)
```
yarn add github:hostinger-bot/Baileys
```

Then import your code using:
``` ts 
import makeWASocket from 'baileys-api'
```
...
``` ts
import makeWASocket, { DisconnectReason } from 'baileys-api'
...
``` ts
import makeWASocket, { BufferJSON, useMultiFileAuthState } from 'baileys-api'
...
``` ts
import makeWASocket, { makeInMemoryStore } from 'baileys-api'
...
``` ts
import { MessageType, MessageOptions, Mimetype } from 'baileys-api'
...
- **MiscGenerationOptions**: some extra info about the message. It can have the following __optional__ values:
    ...
    const info: MessageOptions = {
        ...
        mimetype: Mimetype.pdf, /* (for media messages) specify the type of media (optional for all media types except documents),
                                    import {Mimetype} from 'baileys-api'
                                */
...
import { downloadMediaMessage } from 'baileys-api'
